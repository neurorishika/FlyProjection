...
def camera_delete_file(nodemap):
    ...
    try:
        ...
        if not PySpin.IsReadable(ptr_file_operation_selector) or not PySpin.IsWritable(ptr_file_operation_selector):
...
def camera_open_file(nodemap):
    ...
    try:
        ...
        if not PySpin.IsReadable(ptr_file_open_mode) or not PySpin.IsWritable(ptr_file_open_mode):
        ...
        if not PySpin.IsWritable(ptr_file_access_offset):
...
def upload_file_to_camera(nodemap, file_selector_entry_name, file_path):
    ...
    if not PySpin.IsReadable(ptr_file_selector) or not PySpin.IsWritable(ptr_file_selector):
    ...
    ptr_file_access_buffer = PySpin.CRegisterPtr(nodemap.GetNode('FileAccessBuffer'))
    if not PySpin.IsWritable(ptr_file_access_buffer):
        print('Unable to query FileAccessBuffer. Aborting...')
        return False
...
def configure_inference(nodemap, is_enabled):
    ...
    try:
        if is_enabled:
            ...
            if not PySpin.IsReadable(ptr_inference_network_type_selector) or not PySpin.IsWritable(ptr_inference_network_type_selector):
...
def configure_test_pattern(nodemap, is_enabled):
    ...
    try:
        ...
        if not PySpin.IsReadable(ptr_test_pattern) or not PySpin.IsWritable(ptr_test_pattern):
...
def acquire_images(cam, nodemap, nodemap_tldevice):
    ...
    try:
        ...
        if not PySpin.IsReadable(ptr_acquisition_mode) or not PySpin.IsWritable(ptr_acquisition_mode):
