...
def configure_lookup_tables(nodemap):
    ...
    try:
        ...
        if not PySpin.IsReadable(lut_selector) or not PySpin.IsWritable(lut_selector):
            print_retrieve_node_failure("node", "LUTSelector")
            return False

        lut_selector_lut1 = lut_selector.GetEntryByName("LUT1")
        if not PySpin.IsReadable(lut_selector_lut1):
        ...
        if not PySpin.IsReadable(lut_value) or not PySpin.IsWritable(lut_value):
        ...
        if not PySpin.IsWritable(lut_index):
        ...
        if not PySpin.IsWritable(lut_enable):
