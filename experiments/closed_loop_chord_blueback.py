...
def attempt_engagement_chord(
    ...
    angle_diff = abs(
        np.arctan2(
            np.sin(estimate["direction"] - desired_angle),
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fly Behavior Recording with Chord Engagement"
    )
    parser.add_argument(
        "--repo_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Total duration of the experiment in minutes",
    )
    parser.add_argument(
        "--n_targets",
        type=int,
        default=1,
        help="Number of targets to track",
    )
    ...
    # Initialize n_targets from command line args
    ...
    datasets_config = {
        "data": {
            "shape": (n_targets,),  # One row per target per frame
            "dtype": data_dtype,
        }
    }
    global_metadata = {
    ...
    # Ask user how many targets to track (can override command line argument)
    n_targets_input = get_string_answer_qt(
        "Enter number of targets to track:", str(n_targets)
    )
    if n_targets_input:
        n_targets = int(n_targets_input)
    ...
    ) as saver, manager as manager, FastTracker(
        camera=camera, n_targets=n_targets, debug=args.display, smoothing_alpha=0.1
    ) as tracker, Artist(
            ...
            while True:
                ...
                if estimates is not None:
                    ...
                    for estimate in estimates:
                        ...
                        vector_to_center = np.array(center) - np.array(
                            estimate["position"]
                        )
                        desired_angle = np.arctan2(
                            vector_to_center[1], vector_to_center[0]
                        )
                        angle_diff = abs(
                            np.arctan2(
                                np.sin(estimate["direction"] - desired_angle),
                        ...
                        if in_task[estimate["id"]] and not engagement[estimate["id"]]:
                            ...
                            if chord is not None:
                                ...
                                logger.info(
                                    f"Engagement triggered for ID {estimate['id']}"
                                )
                        else:
                            ...
                            if (
                                time_since_last_encounter[estimate["id"]] > 3
                            ):  # e.g., 3 seconds timeout
                        ...
                        if engagement[estimate["id"]]:
                            ...
                            eng_path = (
                                np.array(engaged_paths[estimate["id"]])
                                .round()
                                .astype(np.uint16)
                            )
                        else:
                            eng_path = (
                                np.ones((path_steps, 2), dtype=np.uint16) * np.nan
                            )
