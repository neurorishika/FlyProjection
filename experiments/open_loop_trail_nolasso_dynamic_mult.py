...
if __name__ == "__main__":
    ...
    # setup expanding trail parameters
    start_radius_mm = 5
    end_radius_mm = 75
    expansion_duration = 10  # seconds for full expansion
    spawn_trigger_fraction = 0.6  # spawn next circle when previous reaches 60% of expansion

    start_radius = start_radius_mm * rig_config["physical_to_camera_scaling"]
    end_radius = end_radius_mm * rig_config["physical_to_camera_scaling"]

    spawn_trigger_radius = start_radius + spawn_trigger_fraction * (end_radius - start_radius)
            ...
            try:
                while not stop_event.is_set():
                    ...
                    if started:
                        ...
                        # Expanding–resetting circle logic
                        # Initialize first circle if none exist
                        
                    if len(circles) == 0:
                        circles.append({"start_time": elapsed_time})

                    # Update circle radii
                    active_radii = []

                    for circle in circles:
                        circle_elapsed = elapsed_time - circle["start_time"]

                        if circle_elapsed <= expansion_duration:
                            growth_fraction = circle_elapsed / expansion_duration
                            radius_i = start_radius + growth_fraction * (end_radius - start_radius)
                        else:
                            radius_i = end_radius

                        circle["radius"] = radius_i
                        active_radii.append(radius_i)

                     # Remove circles that have fully expanded
                    circles = [c for c in circles if c["radius"] < end_radius]

                    # Spawn new circle if last circle passed trigger radius
                    last_circle = circles[-1]
                    if last_circle["radius"] >= spawn_trigger_radius:
                        circles.append({"start_time": elapsed_time})
                        ...
                        # Always draw the trail in open-loop
                    for radius_i in active_radii:
                        background.add_circle(
                            center,
                            radius_i,
                            color=trail_color,
                    ...
                    if estimates is not None:
                        ...
                        for estimate in estimates:
                            ...
                            if len(active_radii) > 0:
                                distance_from_trail = np.min(
                                    [np.abs(polar[0] - r) for r in active_radii]
                                )
                            else:
                            ...
                            # check task status based on trail position
                            if (
