        ...
        if self.detection_initialized:
            for i, _ in enumerate(self.kalman_filters):
                if len(self.trajectories[i]) >= 2:
                    ...
                    if delta_time > 0 and np.all([not np.isnan(current_entry[key]) for key in ['x', 'y']]) and np.all([not np.isnan(prev_entry[key]) for key in ['x', 'y']]):
                        velocity = distance.euclidean(
                            [current_entry['x'], current_entry['y']],
                            [prev_entry['x'], prev_entry['y']]
                        ) / delta_time
                    else:
                        velocity = (np.nan, np.nan)
                    angle = current_entry['theta']
                    estimates.append({
                        'id': self.track_ids[i],
                        'position': (current_entry['x'], current_entry['y']),
                        'velocity': velocity,
                        'angle': angle,
                        'time_since_start': current_entry['time'] - self.start_time
                    })
                else:
